...
def test_validate_user():
    ...
    with testutils.patch_request("app.common.sam", "get", 403):
    ...
    with testutils.patch_request("app.common.sam", "get", 200, json={"userSubjectID": "foo"}):  # missing a bunch of stuff
    ...
    with testutils.patch_request("app.common.sam", "get", 404):
        with pytest.raises(exceptions.ISvcException) as excinfo:
            sam.validate_user("ya29.bearer_token")
            assert excinfo.value.http_status == 403

    # sam returns an error
    with testutils.patch_request("app.common.sam", "get", 500):
        with pytest.raises(exceptions.ISvcException) as excinfo:
            sam.validate_user("ya29.bearer_token")
    ...
    with testutils.patch_request("app.common.sam", "get", 200, json=bad_user):
    ...
    with testutils.patch_request("app.common.sam", "get", 200, json=good_user):
